...
export default function DashboardOverview() {
  ...
  const totalRemainingAmount = emis.reduce(
    (sum, emi) => sum + getRemainingAmount(emi),
    0
  );
  ...
  const upcomingPayments = activeEMIs
    .map((emi) => ({
      ...
      daysUntilPayment: Math.ceil(
        (new Date(emi.nextPaymentDate).getTime() - new Date().getTime()) /
          (1000 * 60 * 60 * 24)
      ),
    }))
  ...
  const { data } = useFetch(getEmi, {}, true);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-600 to-pink-600 text-white overflow-hidden relative">
          ...
          <CardContent className="relative z-10">
            <div className="text-3xl font-bold mb-1">
              {paymentHealthScore.toFixed(0)}/100
            </div>
            <div className="text-sm opacity-75">
            ...
            <CardContent className="space-y-4">
              {upcomingPayments.length > 0 ? (
                upcomingPayments.map((emi) => (
                  <div
                    key={emi.id}
                    className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-blue-100"
                  >
                    ...
                    <div className="text-right">
                      <Badge
                        variant={
                          emi.daysUntilPayment <= 3
                            ? "destructive"
                            : emi.daysUntilPayment <= 7
                            ? "default"
                            : "secondary"
                        }
                      >
                        {emi.daysUntilPayment <= 0
                          ? "Due Now"
                          : `${emi.daysUntilPayment} days`}
                      </Badge>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t">
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-600">
                    {Math.round(
                      activeEMIs.reduce(
                        (sum, emi) => sum + getMonthsRemaining(emi),
                        0
                      ) / Math.max(activeEMIs.length, 1)
                    )}
                  </div>
